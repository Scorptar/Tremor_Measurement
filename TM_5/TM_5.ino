...
void WriteSRAM()
{
  union ByteSplit                                                       // union class
    ...
    SRAM5.writeByte(START_ADDRESS_WRITE_5,ValueX5.int8[1]); 
    SRAM5.writeByte(START_ADDRESS_WRITE_5+1,ValueX5.int8[0]);

    ByteSplit ValueY5;
    ValueY5.int16= YValue; 
    SRAM5.writeByte(START_ADDRESS_WRITE_5+2,ValueY5.int8[1]); 
    SRAM5.writeByte(START_ADDRESS_WRITE_5+3,ValueY5.int8[0]); 
    

    ByteSplit ValueZ5;
    ValueZ5.int16= ZValue; 
    SRAM5.writeByte(START_ADDRESS_WRITE_5+4,ValueZ5.int8[1]); 
    SRAM5.writeByte(START_ADDRESS_WRITE_5+5,ValueZ5.int8[0]); 
...
void ReadSRAM()
{
  ...
  Serial.flush(); 
  Serial.print(",");
  MSB=SRAM5.readByte(START_ADDRESS_READ_5); 
  LSB=SRAM5.readByte(START_ADDRESS_READ_5+1); 
  wd = ((int16_t)MSB << 8) | (LSB & 0xFF);
  wd = (wd/1024)*9.81*1000; 
  Serial.flush(); 
  Serial.print(wd);
  ...
  Serial.flush(); 
  Serial.print(",");   
  MSB=SRAM5.readByte(START_ADDRESS_READ_5+2);
  LSB=SRAM5.readByte(START_ADDRESS_READ_5+3);
  wd = ((int16_t)MSB << 8) | (LSB & 0xFF);
  wd = (wd/1024)*9.81*1000; 
  Serial.flush(); 
  Serial.print(wd); 
  ...
  Serial.flush(); 
  Serial.print(",");
  MSB=SRAM5.readByte(START_ADDRESS_READ_5+4); 
  LSB=SRAM5.readByte(START_ADDRESS_READ_5+5); 
  wd = ((int16_t)MSB << 8) | (LSB & 0xFF);
  wd = (wd/1024)*9.81*1000; 
  Serial.flush(); 
  Serial.println(wd);
